...
#Added to output for visualization exercise in D3
...
Links are available, as provided by the Oakland Police Department ([here here](http://oakgis.maps.arcgis.com/apps/OnePane/basicviewer/index.html?appid=69801e23ca8e456fbe6af18d2e88c614))
...
```{r Exploration - crime incidence by geography over time (c)}
...
crimes <- transform(crimes, mainBeat = ifelse((Beat %in% topBeats), 
                                              as.character(Beat), 'other'))

crime_by_month_and_main_beat <- subset(crimes, date_format < cutOffDate) %>%
...
ggplot(aes(x= month, y=incidents), data=crime_by_month_and_main_beat) +
  geom_line(aes(color = mainBeat)) +
  facet_wrap(~ mainBeat, scales='free_y') +
  geom_smooth()
summary(crime_by_month_and_main_beat)
...
```{r Exploration - crime heatmap reference, fig.width=20, fig.height=16}

oakland_roadmap_location <- c(-122.326, 37.74, -122.17, 37.85)
oakland_roadmap <- get_map(location = oakland_roadmap_location, 
...
```{r Exploration - crime heatmap reference - 2014 vs 2008, fig.width=30, fig.height=24}

crimes_two_o_eight <- subset(crimesMapSubset, date_format >= '2008-01-01' & 
                              date_format < '2009-01-01' )
crimes_two_fourteen <- subset(crimesMapSubset, date_format >= '2014-01-01' & 
                              date_format < '2015-01-01' )

two_o_eight_map <- ggmap(oakland_roadmap) +
        stat_density2d(data = crimes_two_o_eight, aes(x = Lng, y = Lat, 
                        fill = ..level.., alpha = ..level..), 
                        size = 0.2, bins = 20, geom = 'polygon', 
                        limits=c(0,1000),breaks=seq(0,1000,by=100)) +
...
two_fourteen_map <- ggmap(oakland_roadmap) +
        stat_density2d(data = crimes_two_fourteen, aes(x = Lng, y = Lat, 
                      fill = ..level.., alpha = ..level..), 
                      size = 0.2, bins = 20, geom = 'polygon', 
                      limits=c(0,1000),breaks=seq(0,1000,by=100)) +
        scale_fill_gradient(low = "blue", high = "red", limits=c(0,1000)) +
...
```{r Exploration - distribution of crime rates by beat}
...
ggplot(aes(x=week, y=log(incidents)), data=crime_by_week) +
  ...
  ylab("Crimes Reported Per Week (Log-scale)") +
  xlab("Date") +
  scale_y_continuous(breaks = seq(6,8,0.25), labels = comma) +
  stat_smooth(method = 'lm', formula = I(y)~I(as.numeric(x)),  color = 'red') +
  ggtitle("Weekly Crime Incidence in Oakland:\n January 2007 to June 2015") +
  theme(plot.title = element_text(lineheight=1, face="bold", size = 30)) +
  theme(axis.title.y = element_text(lineheight=1, face="bold", size=24, 
                                    vjust=1.5)) +
  theme(axis.title.x = element_text(lineheight=1, face="bold", size=24, 
                                    vjust=0.5),
...
```{r Final Plot 1b - Aggregate Trends in Crime Rates, include=FALSE}
...
```{r Final Plot 2a - Trends in Crime Rates by Crime Type, fig.width=20, fig.height=16}
fmt <- function(){
...
ggplot(aes(x=month, y=log(incidents)), data=crime_types_by_month) +
  ...
  ylab("Crimes Report Per Month (Log Scale)") +
  xlab("Date") +
  ...
  ggtitle("Monthly Crime Incidence in Oakland by Crime Category:\n 
          January 2007 to June 2015") +
  theme(plot.title = element_text(lineheight=0.5, face="bold", size=30, 
                                  vjust=1),
        axis.title.y = element_text(lineheight=1, face="bold", size=24, 
                                    vjust=1.5),
        axis.title.x = element_text(lineheight=1, face="bold", size=24, 
                                    vjust=0.5),
        strip.text.x = element_text(size = 14, colour = "blue", 
                                    face='bold'),
        axis.text = element_text(size = 16),
        legend.title = element_text(size = 24),
        legend.text = element_text(size = 22, lineheight = 1.5),
...
```{r Final Plot 2b - Trends in Crime Rates by Crime Type, include=FALSE}

cm1 <- lm(I(log(incidents)) ~ I(as.numeric(month)), 
...
### Final Plot 3: View of trends in median, mean and quantile measures of monthly crime by police beat

For the third chart, I decided to clean up my line plot of monthly crime rate centrality measures over police beats, to better highlight the key features of similar plots I tried in the earlier exploration phase.

```{r Final Plot 3a - Trends in Crime Rates by Police Beat Crime Rate Quantile, fig.width=20, fig.height=16}

crime_beat_distribution_by_month <- crime_by_month_and_beat %>%
  group_by(month) %>%
  summarize(mean = mean(incidents),
            median = median(incidents),
            top_decile = quantile(incidents, 0.9),
            bottom_decile = quantile(incidents, 0.1)) %>%
  arrange(month)

cbd.melt <- melt(crime_beat_distribution_by_month, id = "month", 
                 variable.name = "measure")

ggplot(cbd.melt, aes(x = month, y = value, group = measure)) +
  geom_line(aes(color = measure), size = 1) +
  stat_smooth(method = 'lm', formula = I(y)~I(as.numeric(x)), 
              color = 'lightblue', alpha = 0.2, size = 0.3) +
  scale_color_brewer(type = "qual", palette = 2, 
                     labels = c("Beat Mean", "Beat Median", 
                                "Top Decile of Beats", 
                                "Bottom Decile of Beats"),
                     name = "Beat Group (by Crime Rate)") +
  ggtitle("Monthly Crime Reports for Oakland Police Beats:\n 
          January 2007 to June 2015") +
  ylab("Crime Reports Per Month") +
  xlab("Date") +
  theme(plot.title = element_text(lineheight=0.5, face="bold", size = 30, 
                                  vjust=1.5),
        axis.title.y = element_text(lineheight=1, face="bold", size=24, 
...
To test how the inclusion of crime type variables would change our trend line, I decided to also include `Beat` as a independent variable in a linear regression model of `log(incidents)` versus month. The addition seems to raise trend fit versus a model of just incidents vs time, with R-squared for the new model at 0.88. By conditioning trends on police beat we see a lower average rate of crime reduction than with overall data. With the linear model here, it looks like crime for the typical police beat is diminishing at a rate of around 9.5% per year. 

```{r Final Plot 3c - Trends in Crime Rates by Police Beat Crime Rate Quantile, include=FALSE}

cmb1 <- lm(I(log(incidents)) ~ I(as.numeric(month)), 
           data = subset(crime_by_month_and_beat, incidents > 0))
cmb2 <- update(cmb1, ~ . + Beat)
summary(cmb2)
